...
if(window.jQuery&&$('.heroSlider').length){$('.heroSlider').slick({autoplay:true,autoplaySpeed:5000,arrows:true,dots:true,pauseOnHover:true,pauseOnFocus:true,adaptiveHeight:false,prevArrow:'<button type="button" class="slickArrow slickPrev" aria-label="이전 슬라이드">←</button>',nextArrow:'<button type="button" class="slickArrow slickNext" aria-label="다음 슬라이드">→</button>'});}
